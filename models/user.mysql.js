...
class User {
  ...
  static async getAllUsers() {
    const sql = `SELECT id, username, email, phone, fullname, main_uid FROM users`;
    try {
  ...
  static async findByPhone(phone) {
    ...
    try {
      ...
      throw new Error('Error fetching user by phone: ' + err.message);
    }
  }
  ...
  static async updateUserDetails(id, username, email, phone, password, fullname) {
    const sql = `
      UPDATE users
      SET username = ?, email = ?, phone = ?, password = ?, fullname = ?
      WHERE id = ?`;

    try {
      return await db.query(sql, [username, email, phone, password, fullname, id]);
    } catch (err) {
  ...
  static async updateRoles(id, roleIds) {
    const sql = `DELETE FROM user_roles WHERE uid = ?`;
    try {
      ...
      throw new Error('Error updating user roles: ' + err.message);
    }
  }
  ...
  static async removeRoles(id) {
    ...
    try {
      ...
      throw new Error('Error removing user roles: ' + err.message);
    }
  }
